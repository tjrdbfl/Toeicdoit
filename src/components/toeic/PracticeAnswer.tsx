...
const PracticeAnswer = ({
    ...
    return (
        <div className="bg-white border-slate-200 border-2 shadow-lg rounded-lg w-[200px] h-[700px] m-5 lg:mr-[25%]">
            <div className="w-full p-2">
                <div className="bg-white border-slate-200 border-2 text-black font-medium text-center text-lg py-1 shadow-md rounded-md w-full">
                    {label}
                </div>
            </div>

            <form onSubmit={handleSubmit} method="POST">
                <ul className="mt-2 m-2 p-2 border-slate-200 border-2 w-[180px] h-[570px] rounded-lg shadow-md">
                    <ScrollArea className="transition-opacity duration-200 text-black w-full h-full">
                        {Array.from({ length: count }, (_, index) => index + 1).map((questionNumber) => (
                            ...
                            >
                                <Link
                                    href={`/level/${id}?page=${questionNumber}`}
                                    key={`question-${questionNumber}`}
                                    className={`text-black hover:text-zinc-500 hover:underline text-lg font-semibold w-[45px] border-slate-200 border-r-2 py-1 px-2 bg-blue-50`}
                                >
                                    {questionNumber}
                                </Link>
                                    ...
                                    {(part === 2 ? option2 : option1).map((option) => (
                                        <div key={`radio-fragment-${questionNumber}-${option}`} className="relative">
                                            ...
                                            <label
                                                ...
                                                className={`text-black w-4 h-4 text-lg rounded-full ring-1 ring-black flex items-center justify-center cursor-pointer 
                ...
                <div className="mx-10 mt-5">
                    <button 
                    className="w-full h-auto py-[2.5%] flex text-center items-center text-white shadow-xl rounded-lg bg-black justify-center text-[18px] font-bold hover:bg-zinc-700"
                    >제출하기</button>
                </div>
            </form>
        </div>
