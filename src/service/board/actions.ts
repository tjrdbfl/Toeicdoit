...
export async function modifyBoard(prevState: FreeMessageState, formData: FormData) {
    ...
    if (checkResposnse?.message === 'LOGOUT') {
        ...
        try {
            ...
            if (result.message === 'SUCCESS') {
                revalidatePath(`${PG.INQUIRY_DETAILS}`);
                revalidatePath(`${PG.INQUIRY_DETAILS}/modify/${boardId}`);
    ...
    if (checkResposnse?.message === 'LOGOUT') {
        ...
        if (accessToken === undefined || userId === undefined) {
            ...
            if (replyId === undefined || boardId === undefined) {
                ...
                try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_USER_API_URL}/${SERVER_API.REPLY}/delete?id=${replyId}`, {
                        method: 'DELETE',
                        cache: 'no-store',
                        headers: AuthorizeHeader(accessToken)
                    })
    ...
    if (checkResposnse?.message === 'LOGOUT') {
        ...
        if (accessToken === undefined || userId === undefined) {
            ...
            if (replyId === undefined || content === undefined) {
                ...
                try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_USER_API_URL}/${SERVER_API.REPLY}/modify-by-content`, {
                        method: 'PUT',
                        cache: 'no-store',
                        headers: AuthorizeHeader(accessToken),
                        body:JSON.stringify({
