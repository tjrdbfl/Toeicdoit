import { StudyRecipe1 } from "@/constants/study/faq";

export default function StudyFAQRecipe2() {
  return (
    <>
    <h1 className="mt-10 text-lg font-semibold  text-blue-500">무조건 많은 문제를 풀기 보다는, 버리는 스킬이 더 중요!</h1>
    ...
    <p className="text-black text-pretty text-[13px] mt-1">그리고 점진적으로 신규 문제를 줄이고 1일 1복습으로 실전력을 올려주세요.</p>
     ...
     <div className="bg-gradient-faq1 rounded-xl border-blue-100 border-2 py-10 px-5 lg:px-20 mt-5">
        ...
        <div className="w-full rounded-t-lg text-black flex flex-row border-slate-100 border-b-2 gap-x-1 border-b-zinc-200 ">
          <p className="w-[25%] text-center font-semibold  text-[14px] p-2">파트</p>
          <p className="w-[25%] text-center font-semibold text-[14px] bg-gradient-faq2 p-2 rounded-t-lg border-zinc-200 border-2">
           미상승
          </p>
          <p className="w-[25%] text-center font-semibold text-[14px]  bg-gradient-faq2 p-2 rounded-t-lg border-zinc-200 border-2">
            100점 미만 상승
          </p>
          <p className="w-[25%] text-center font-semibold text-[14px]  bg-gradient-faq3 p-2 rounded-t-lg border-zinc-200 border-2">100점 이상 상승</p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center w-full">
              {StudyRecipe1.map((item)=>(
                ...
                <p className="w-[25%] font-semibold  text-center   text-[14px] py-2">
                  {item.part}
                </p>
                <p className="w-[25%] bg-gradient-faq4 text-center font-semibold text-[14px] py-2 2">
                  {item.col1}
                </p>
                <p className="w-[25%] bg-gradient-faq4 text-center font-semibold text-[14px] py-2 ">
                 {item.col2}
                </p>
                <p className="w-[25%] bg-gradient-faq5 text-center text-blue-500 text-[14px] font-semibold  py-2 ">
                  {item.col3}
                </p>
              </div>
